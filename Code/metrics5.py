...
class Metrics(object):
    ...
    def plot_per_phantom(self, phantom, ssim_per_slice, mse_per_slice, dsc_per_slice, psnr_per_slice, x1, x2):
        ...
        #return (ssim_plot, mse_plot, dsc_low_plot, dsc_high_plot, psnr_plot)
    ...
    def calculate_metrics(self):
        ...
        for z, phantom in enumerate(self.phantoms):
            for it_idx, ii in enumerate(self.it):
               # gold_standard_imgs = natsorted(glob('/bigstore/felix/WalnutsRadialSlices/Walnut{}/iterative_iter*'.format(phantom)))
                # input_imgs = natsorted(glob('/bigstore/felix/WalnutsRadialSlices/Walnut{}/fdk_pos{}_*'.format(phantom, pos)))
                #ir_imgs = natsorted(glob('/bigstore/felix/WalnutsRadialSlices/Walnut{}/iterative_pos{}*'.format(phantom, pos)))
                # cnn_radial = natsorted(
                #     glob(path_to_radial_imgs + '/{}_pos{}_it{}_*_phantom{}/*'.format(self.architecture, pos, ii, phantom)))
                ...
                #rad2ax_imgs = natsorted(glob('/bigstore/jordi/Walnuts/results/predictions/Radial2Axial/{}_it{}_phantom{}/*'.format(self.architecture, ii, phantom)))
                if self.architecture == 'unet_jordi' or 'unet':
                    arch='unet'  
                it_hor=ii
                #if self.architecture== 'unet_jordi':
                it_hor=2              
                hor_imgs = natsorted(
                    glob(self.path_to_hor_imgs + '{}_pos{}_it{}_*_phantom{}/*'.format(arch, self.pos, it_hor, phantom)))

                #hor_imgs = natsorted(glob('/bigstore/jordi/Walnuts/results/predictions/Horizontal/{}_pos{}_it{}_*_phantom{}/*'.format(self.architecture, pos, ii, phantom)))
                print(len(gold_standard_imgs), len(input_imgs), len(rad2ax_imgs),  len(hor_imgs), len(ir_imgs))
                ...
                #volumes =  [inp_vol, horizontal_vol, rad2ax_vol]


                for j, img in enumerate(imgs):
                    print(j, 'j', len(img))
                    # Calculate metrics per slic
           ...
           # print(SSIM.shape, PSNR.shape, DSC.shape)

            #slices=[ 100,200, 300, 310, 315, 350]
            slices=[100]
            if len(slices)>0:
                for slice in slices:
            ...
            make_per_phantom_plots= True  # if not true, we can load the txt results?
            ...
            #    print(otsu_gs, otsu_hor, otsu_rad2ax, 'otsus')
...
'''
# # test parameters
phantoms = [9, 13]  #,13,16,19,33,36,37  #[35,27,15,3,39,7,33]
architecture='msd'
pos=1
it=1 #7,8,10
# mode='horizontal'
